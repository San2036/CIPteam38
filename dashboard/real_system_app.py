    ...
    def connect_blockchain(self):
        ...
        try:
            if self.w3.is_connected():
                # Load Contract Address dynamically generated by Node 1
                base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                address_file = os.path.join(base_dir, 'contract_address.json')
                
                if not os.path.exists(address_file):
                    st.error("waiting for Node 1 to deploy smart contract...")
                ...
                # Hardcoded ABI to avoid dependency on main_node and flask
                abi = [
                    {"inputs": [], "stateMutability": "nonpayable", "type": "constructor"},
                    {"anonymous": False, "inputs": [{"indexed": True, "internalType": "address", "name": "nodeAddress", "type": "address"}, {"indexed": False, "internalType": "string", "name": "newHash", "type": "string"}, {"indexed": False, "internalType": "bytes", "name": "proof", "type": "bytes"}, {"indexed": False, "internalType": "uint256", "name": "timestamp", "type": "uint256"}], "name": "ModelUpdated", "type": "event"},
                    {"anonymous": False, "inputs": [{"indexed": True, "internalType": "address", "name": "badNode", "type": "address"}, {"indexed": False, "internalType": "uint256", "name": "timestamp", "type": "uint256"}], "name": "NodeSlashed", "type": "event"},
                    {"inputs": [{"internalType": "uint256", "name": "nodeId", "type": "uint256"}], "name": "registerNode", "outputs": [], "stateMutability": "nonpayable", "type": "function"},
                    {"inputs": [{"internalType": "string", "name": "newHash", "type": "string"}, {"internalType": "bytes", "name": "proof", "type": "bytes"}], "name": "uploadUpdate", "outputs": [], "stateMutability": "nonpayable", "type": "function"},
                    {"inputs": [], "name": "getGlobalModelHash", "outputs": [{"internalType": "string", "name": "", "type": "string"}], "stateMutability": "view", "type": "function"},
                    {"inputs": [{"internalType": "address", "name": "nodeAddr", "type": "address"}], "name": "getNodeInfo", "outputs": [{"components": [{"internalType": "uint256", "name": "id", "type": "uint256"}, {"internalType": "uint256", "name": "reputation", "type": "uint256"}, {"internalType": "bool", "name": "isBanned", "type": "bool"}, {"internalType": "address", "name": "nodeAddress", "type": "address"}], "internalType": "struct FLRegistry.Node", "name": "", "type": "tuple"}], "stateMutability": "view", "type": "function"},
                    {"inputs": [{"internalType": "address", "name": "badNode", "type": "address"}], "name": "slashNode", "outputs": [], "stateMutability": "nonpayable", "type": "function"}
                ]
                        ...
                        if tx.to and tx.to.lower() == contract_address:
                            ...
                            try:
                                ...
                                if function_name == 'uploadUpdate':
                                    ...
                                    # Verify True ZK-Proof
                                    from security.zk_proofs import verify_true_zk_proof
                                    is_valid = verify_true_zk_proof(proof, model_hash)
        ...
        try:
            ...
            if len(train_results) == 5:
                ...
                if isinstance(val_accuracy, (int, float)):
                    ...
                    st.error(f"Metrics Type Error: val_accuracy is {type(val_accuracy)}")
                ...
                if len(all_node_weights) >= 2:  # Aggregate when at least 2 banks have trained
                    st.write(f"Merging intelligence from {len(all_node_weights)} banks...")
                    global_w = aggregator.federated_average(all_node_weights)
                    aggregator.save_global_model(global_w)
                    st.success(f"Federated Aggregation SUCCESS: Global Model updated from {len(all_node_weights)}/3 banks!")
            else:
                st.error(f"Training returned unexpected results format: {len(train_results)} items")
            
            # ATTACK SIMULATION
            if st.session_state.get('attack_mode', False):
                model_hash = "SUSPICIOUS_" + str(uuid.uuid4())
                weights = [0.0] * len(weights) # Poison weights
                st.warning("ATTACK MODE: Generating Poisoned Model Update...")
            ...
            if st.session_state.system_state['blockchain_connected']:
                try:
                    ...
                    # 2. Generate ZK Proof
                    st.write(f"Generating Mathematical ZK-Proof for model weights...")
    ...
    def collect_real_time_data(self):
        ...
        while st.session_state.is_running:
            try:
                ...
                # Dynamically count REAL active nodes by pinging P2P Web-Sockets
                import requests
                alive_nodes = 0
                for i in range(1, 4):  # Check nodes 1, 2, 3
                    node_name = f"Node_{i}"
                    try:
                        res = requests.get(f"http://127.0.0.1:{8000+i}/health", timeout=0.5)
                        if res.status_code == 200:
                            alive_nodes += 1
                            st.session_state.system_state['node_health'][node_name] = np.random.randint(95, 100)
                    except:
                        if node_name in st.session_state.system_state['node_health']:
                            st.session_state.system_state['node_health'][node_name] = 0
                            
                st.session_state.system_state['active_nodes'] = alive_nodes
...
def stop_real_system():
    ...
    st.session_state.is_running = False
    st.warning("Real system monitoring stopped")
...
def display_real_training():
    ...
    with col_b:
        if st.button("Stop Real System", key="main_stop"):
            stop_real_system()
...
def display_real_blockchain():
    st.header("Real Blockchain Activity")
...
def display_real_security(collector):
    st.header("Real Security Monitoring")
    ...
    with col1:
        ...
        st.metric("Trust Agent", trust_status)
        ...
        if st.session_state.system_state['trust_agent_active']:
            alerts.append("Trust Agent monitoring real blockchain events")
        ...
        if alerts:
            ...
            st.warning("No security agents active")
...
def sidebar_real_system():
    ...
    if st.sidebar.button("Stop Real System", key="sidebar_stop"):
        stop_real_system()
...
def display_fraud_detection_tab():
    ...
    with col1:
        ...
        prev_activity = st.slider("Previous Fraudulent Activity", 0, 10, 0)
        risk_score = st.slider("Risk Score (internal)", 0, 100, 10)
    ...
    with col2:
        ...
        if st.button("Analyze Transaction", type="primary"):
            ...
            model = create_model(input_size=17, hidden_size=4)
            ...
            if weights is not None:
                ...
                st.warning("Training not complete. Using heuristic decision.")
